...
describe('GameCard', () => {
  ...
  it('backs the title with a pill only when it sits on artwork', () => {
    expect(
      renderToStaticMarkup(<GameCard name="Yazy" art="/y.png" />),
    ).toContain('bg-eo-paper/85');
    expect(renderToStaticMarkup(<GameCard name="Yazy" />)).not.toContain(
      'bg-eo-paper/85',
    );
  });
...
describe('PlayerChip', () => {
  ...
  it('shows a status dot only when a status is given', () => {
    expect(renderToStaticMarkup(<PlayerChip name="Ada" />)).not.toContain(
      'bg-eo-live',
    );
    expect(
      renderToStaticMarkup(<PlayerChip name="Ada" status="online" />),
    ).toContain('bg-eo-live');
  });
});
...
describe('Tabs', () => {
  ...
  it('accepts bare strings and value/label pairs alike', () => {
    const html = renderToStaticMarkup(
      <Tabs
        tabs={['All', { value: 'dice', label: 'Dice games' }]}
        value="dice"
      />,
    );
