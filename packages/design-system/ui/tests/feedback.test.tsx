...
describe('Dialog', () => {
  ...
  it('centres itself against the preflight margin reset', () => {
    expect(renderToStaticMarkup(<Dialog open title="Rematch?" />)).toContain(
      'm-auto',
    );
  });
  ...
  it('carries no close button, leaving Escape, the backdrop and the footer', () => {
    const html = renderToStaticMarkup(
      <Dialog open title="Rematch?" onClose={() => {}} />,
    );
...
describe('Toast', () => {
  ...
  it('shows the dismiss control only when a handler is given', () => {
    expect(renderToStaticMarkup(<Toast message="Saved" />)).not.toContain(
      'aria-label="Dismiss"',
    );
    expect(
      renderToStaticMarkup(<Toast message="Saved" onDismiss={() => {}} />),
...
describe('Tooltip', () => {
  it('keeps the label in the markup and reveals it on hover or focus', () => {
    const html = renderToStaticMarkup(
      <Tooltip label="Copy link">trigger</Tooltip>,
    );
  ...
  it('flips the offset for bottom placement', () => {
    expect(
      renderToStaticMarkup(<Tooltip label="x" placement="bottom" />),
    ).toContain('top-[calc(100%+8px)]');
    expect(renderToStaticMarkup(<Tooltip label="x" />)).toContain(
      'bottom-[calc(100%+8px)]',
    );
  });
});
