...
describe('Flex', () => {
  it('is a bare flex row until told otherwise', () => {
    expect(renderToStaticMarkup(<Flex>x</Flex>)).toContain(
  ...
  it('appends className last so callers can add responsive overrides', () => {
    const html = renderToStaticMarkup(
      <Flex className="max-md:flex-col" direction="row" />,
    );

    expect(html.indexOf('flex-row')).toBeLessThan(
      html.indexOf('max-md:flex-col'),
    );
  });
});
