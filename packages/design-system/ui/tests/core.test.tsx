...
describe('Card', () => {
  it('lifts only when interactive', () => {
    expect(renderToStaticMarkup(<Card>flat</Card>)).not.toContain(
      'hover:-translate-y-px',
    );
    expect(renderToStaticMarkup(<Card interactive>lifts</Card>)).toContain(
      'hover:-translate-y-px',
    );
  });
});
...
describe('Badge', () => {
  it('renders the status dot only when asked', () => {
    expect(renderToStaticMarkup(<Badge tone="live">live</Badge>)).not.toContain(
      'bg-current',
    );
    expect(
...
describe('Tag', () => {
  it('is disabled until it is given a handler', () => {
    ...
    expect(
      renderToStaticMarkup(<Tag onClick={() => {}}>All</Tag>),
...
describe('IconButton', () => {
  it('carries its accessible name and hides the glyph from assistive tech', () => {
    const html = renderToStaticMarkup(
      <IconButton icon={<Icon icon={Swords} />} label="Start match" />,
    );
