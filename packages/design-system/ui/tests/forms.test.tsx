...
describe('Input', () => {
  it('shows the hint until an error replaces it', () => {
    expect(
      renderToStaticMarkup(<Input hint="Room codes are 4 letters" />),
    ).toContain('Room codes are 4 letters');
...
describe('Checkbox', () => {
  it('renders the tick only when checked', () => {
    expect(renderToStaticMarkup(<Checkbox label="Ready" />)).not.toContain(
      '<svg',
    );
    expect(
      renderToStaticMarkup(
        <Checkbox label="Ready" checked onChange={() => {}} />,
      ),
    ).toContain('<svg');
  });
...
describe('Radio', () => {
  it('groups by name and fills the dot when checked', () => {
    const html = renderToStaticMarkup(
      <Radio name="seat" value="p0" checked onChange={() => {}} label="Red" />,
    );
...
describe('Switch', () => {
  it('exposes the switch role and slides the knob when on', () => {
    expect(renderToStaticMarkup(<Switch label="Sound" />)).toContain(
      'role="switch"',
    );
    expect(
      renderToStaticMarkup(
        <Switch label="Sound" checked onChange={() => {}} />,
      ),
    ).toContain('translate-x-5');
    expect(renderToStaticMarkup(<Switch label="Sound" />)).not.toContain(
      'translate-x-5',
    );
  });
});
