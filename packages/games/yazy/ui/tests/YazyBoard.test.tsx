...
describe('YazyBoard', () => {
  ...
  it("shows the acting player's preview to their opponent as well", () => {
    const html = render(
      snapshotOf({ turn: 'p0', dice: [3, 3, 3, 3, 3] }),
      'p1',
    );
  ...
  it('drops the turn underline once the game is over', () => {
    const html = render(
      snapshotOf({ result: { draw: true }, phase: 'over' }),
      'p0',
    );
  ...
  it('leaves the button wordless for the player who cannot act', () => {
    for (const rollsLeft of [2, 0]) {
      expect(render(snapshotOf({ turn: 'p1', rollsLeft }), 'p0')).not.toContain(
        'Roll',
      );
    }
  });
  ...
  it('leaves the button wordless once the game is over', () => {
    expect(
      render(snapshotOf({ result: { draw: true }, phase: 'over' }), 'p0'),
    ).not.toContain('Roll');
  });
});
