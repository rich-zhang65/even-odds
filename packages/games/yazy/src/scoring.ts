...
export const CATEGORY_INFO: Record<Category, CategoryInfo> = {
  ...
  threeOfAKind: {
    label: '3 of a Kind',
    description: 'Sum of all dice (need 3 of same)',
  },
  fourOfAKind: {
    label: '4 of a Kind',
    description: 'Sum of all dice (need 4 of same)',
  },
  fullHouse: {
    label: 'Full House',
    description: '25 pts — 3 of one, 2 of another',
  },
  straight: {
    label: 'Straight',
    description: '40 pts — 5 consecutive numbers',
  },
...
export const scoreCategory = (
  ...
  switch (category) {
    ...
    case 'threeOfAKind':
      return isJoker || maxCount(counts) >= 3 ? total : 0;
    case 'fourOfAKind':
      return isJoker || maxCount(counts) >= 4 ? total : 0;
    case 'fullHouse':
      return isJoker || isFullHouse(counts) ? 25 : 0;
    case 'straight':
      return isJoker || hasStraight(dice) ? 40 : 0;
    case 'yazy':
      return isYazy(dice) ? 50 : 0;
    default:
