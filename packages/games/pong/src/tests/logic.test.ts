...
describe('Pong — serving', () => {
  it('holds the ball on the spot until the delay runs out', () => {
    const waiting = run(
      board({ serve: { inMs: SERVE_DELAY_MS, toward: 'p0' } }),
      3,
    );
  ...
  it('sends the ball toward whoever it is served to', () => {
    ...
    expect(
      run(board({ serve: { inMs: SERVE_DELAY_MS, toward: 'p0' } }), ticks).ball
        .velocity.y,
    ).toBeGreaterThan(0);
    expect(
      run(board({ serve: { inMs: SERVE_DELAY_MS, toward: 'p1' } }), ticks).ball
        .velocity.y,
    ).toBeLessThan(0);
  });
  ...
  it('serves at an angle that is neither flat nor steep', () => {
    const ticks = Math.ceil(SERVE_DELAY_MS / STEP_MS);
    const { velocity } = run(
      board({ serve: { inMs: SERVE_DELAY_MS, toward: 'p1' } }),
      ticks,
    ).ball;
...
describe('Pong — paddles', () => {
  ...
  it('clamps an aim that would put the paddle through the wall', () => {
    expect(
      Pong.reduce(board(), { type: 'AIM', x: 1000 }, 'p0', context()).paddles
        .p0,
    ).toBe(MAX_X);
    expect(
      Pong.reduce(board(), { type: 'AIM', x: -1000 }, 'p0', context()).paddles
  ...
  it('refuses an aim that is not a real number', () => {
    expect(
      Pong.isLegal(board(), { type: 'AIM', x: Number.NaN }, 'p0', context()),
    ).toBe(false);
    expect(
      Pong.isLegal(board(), { type: 'AIM', x: Infinity }, 'p0', context()),
    ).toBe(false);
    expect(Pong.isLegal(board(), { type: 'AIM', x: 30 }, 'p0', context())).toBe(
      true,
    );
  });
});
...
describe('Pong — the ball', () => {
  ...
  it('speeds up on every return, up to a ceiling', () => {
    ...
    expect(
      Math.hypot(state.ball.velocity.x, state.ball.velocity.y),
    ).toBeLessThanOrEqual(BALL.maxSpeed + 1e-9);
  });
});
...
describe('Pong — scoring', () => {
  ...
  it('ends the match at the target score', () => {
    ...
    expect(
      Pong.isTerminal(board({ scores: { p0: TARGET_SCORE - 1, p1: 0 } })),
    ).toBeNull();
    expect(
      Pong.isTerminal(board({ scores: { p0: TARGET_SCORE, p1: 2 } })),
    ).toEqual({
      winner: 'p0',
    });
    expect(
      Pong.isTerminal(board({ scores: { p0: 2, p1: TARGET_SCORE } })),
    ).toEqual({
      winner: 'p1',
    });
  });
...
describe('Pong — determinism', () => {
  it('lands in the same place from the same seed and the same inputs', () => {
    const play = (): PongState => {
      ...
      for (let i = 0; i < 400; i++) {
        if (i === 20)
          state = Pong.reduce(state, { type: 'AIM', x: 70 }, 'p0', ctx);
        if (i === 90)
          state = Pong.reduce(state, { type: 'AIM', x: 25 }, 'p1', ctx);
  ...
  it('picks the opening serve from the seed, and both sides are reachable', () => {
    const opening = (seed: number): PlayerId =>
      Pong.setup(context(seed)).serve.toward;
